...
func TestA2AGRPCInt32ParamBounds(t *testing.T) {
	client := &A2AGRPCClient{Tenant: "agent-slug"}
	negative := -1
	for _, tc := range []struct {
		...
		{
			name: "get task history length",
			err: func() error {
				_, err := client.GetTask(context.Background(), A2ATaskQueryParams{ID: "task", HistoryLength: &negative})
				return err
			}(),
		},
		{
			name: "list tasks page size",
			err: func() error {
				_, err := client.ListTasks(context.Background(), A2ATaskListParams{PageSize: &negative})
				return err
			}(),
		},
		{
			name: "list tasks history length",
			err: func() error {
				_, err := client.ListTasks(context.Background(), A2ATaskListParams{HistoryLength: &negative})
				return err
			}(),
		},
		{
			name: "send message history length",
			err: func() error {
				_, err := client.SendMessageResponse(context.Background(), A2AMessageSendParams{
					Configuration: &A2ASendConfiguration{HistoryLength: &negative},
				})
				return err
			}(),
		},
		{
			name: "list push configs page size",
			err: func() error {
				_, err := client.ListTaskPushNotificationConfigs(context.Background(), A2ATaskPushConfigParams{ID: "task", PageSize: &negative})
				return err
			}(),
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			if tc.err == nil || !strings.Contains(tc.err.Error(), "between 0") {
				t.Fatalf("bounds error = %v", tc.err)
			}
		})
	}
	...
	if strconv.IntSize > 32 {
		...
		if _, err := client.ListTasks(context.Background(), A2ATaskListParams{PageSize: &tooLarge}); err == nil || !strings.Contains(err.Error(), "pageSize") {
			t.Fatalf("oversized pageSize error = %v", err)
		}
	}
}
