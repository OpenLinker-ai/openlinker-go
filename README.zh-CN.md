...
下面的极简入口开始，由 SDK 统一管理 Runtime 配置、发现得到的传输安全策略、持久化状态、lease、cancel、
resume、重连、传输切换和优雅关闭。
...
认证、传输安全、权限、payload 和 contract 错误不会触发 fallback。完整分层见
[Runtime 示例](./example/runtime/README.zh-CN.md)；只有明确要自己管理 Runtime 生命周期时，
才从底层协议示例开始。
...
`NewRuntimeWorker` 发现 Runtime 地址及 token-only/mTLS 策略，并负责交付、恢复和持久化。HTTP、command、
...
flowchart LR
  ...
  RuntimeSDK -->|"Agent Token + 发现得到的 TLS 策略 / WebSocket 或 HTTP 长轮询"| Core
...
准备 Agent 身份、Runtime Node、发现得到的安全策略和持久化目录，并完成真实 Run、cancel 和重启验收。
...
`RuntimeWorker` 负责地址和 token-only/mTLS 策略发现、Session、WebSocket/Pull 自动切换、assignment 确认、
...
明确不携带 mTLS 能力的宿主可以设置 `RequireTokenOnly`。如果 discovery 之后要求 mTLS，
`Start` 和策略恢复会返回可匹配 `ErrRuntimeSecurityPolicyUnsupported` 的错误，不会降级、
绕过 discovery 或建立半完成 Session。
...
API 分层、注册、Native handler、Managed Worker 和底层协议入口见
[Runtime 示例](./example/runtime/README.zh-CN.md)。端到端部署与运维见
[RuntimeWorker 完整操作手册](./runtime-worker-end-to-end.zh-CN.md)。
