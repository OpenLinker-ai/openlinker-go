...
func run(ctx context.Context, cfg config, output io.Writer) error {
	...
	runtimeClient, err := openlinker.NewRuntime(cfg.RuntimeBase,
		...
		openlinker.WithSDKAgent("openlinker-go/example/runtime/protocol-websocket"))
	if err != nil {
		return err
	}
	...
	hello := openlinker.RuntimeHelloPayload{
		...
		SessionEpoch: 1, NodeVersion: "openlinker-go/example/protocol-websocket", Capacity: 1,
		Features: openlinker.RuntimeRequiredFeatures(), ContractDigest: openlinker.RuntimeContractDigest,
	}
	connection, err := runtimeClient.DialRuntimeWebSocket(ctx, hello)
	if err != nil {
		return fmt.Errorf("dial WebSocket: %w", err)
	}
	defer connection.Close()
	assignment, err := connection.ClaimRuntimeRun(ctx, 0, openlinker.RuntimeClaimRequest{RuntimeSessionID: sessionID, Capacity: 1})
	if err != nil {
		return fmt.Errorf("receive assignment: %w", err)
	}
	rejected, err := connection.RejectRuntimeAssignment(ctx, openlinker.RuntimeAssignmentRejectPayload{
		AttemptIdentity: assignment.AttemptIdentity, ReasonCode: openlinker.RuntimeRejectNodeDraining, Capacity: 0,
	})
	if err != nil {
		return fmt.Errorf("reject assignment: %w", err)
	}
	return exampleutil.PrintJSON(output, map[string]any{
		"attachment_id": connection.Ready().AttachmentID, "run_id": assignment.AttemptIdentity.RunID,
		"outcome": rejected.Outcome, "warning": "底层 WebSocket 只提供协议原语，不管理 durable spool、resume 或 reconnect",
	})
}
