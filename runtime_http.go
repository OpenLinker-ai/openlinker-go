...
func DecodeRuntimePendingCommand(command RuntimePendingCommand) (RuntimeDecodedPendingCommand, error) {
	...
	case RuntimeBrowserViewerCommand:
		var payload RuntimeBrowserViewerCommandPayload
		if err := decodeRuntimeResponse(bytes.NewReader(command.Payload), &payload); err != nil {
			return RuntimeDecodedPendingCommand{}, err
		}
		if err := validateRuntimeBrowserViewerCommand(payload); err != nil {
			return RuntimeDecodedPendingCommand{}, err
		}
...
func validateRuntimeBrowserViewerCommand(
	value RuntimeBrowserViewerCommandPayload,
) error {
	if err := validateRuntimeAttemptIdentity(value.AttemptIdentity); err != nil ||
		!runtimeUUID(value.BrowserSessionID) ||
		!runtimeUUID(value.AttachmentID) ||
		...
		return errors.New("openlinker: invalid browser Viewer command identity")
	}
	switch value.Action {
	case RuntimeBrowserViewerClaim,
		RuntimeBrowserViewerRelease,
		RuntimeBrowserViewerResume,
		RuntimeBrowserViewerTerminate:
		if value.Input != nil ||
			value.ControlEpoch != value.PreviousControlEpoch+1 {
			return errors.New("openlinker: browser Viewer command contains unexpected input")
		}
	case RuntimeBrowserViewerInput:
		if value.Input == nil ||
			value.ControlEpoch != value.PreviousControlEpoch ||
			!validRuntimeBrowserViewerInput(*value.Input) {
			return errors.New("openlinker: invalid browser Viewer input")
		}
	default:
		return errors.New("openlinker: invalid browser Viewer command action")
	}
	return nil
}

func validRuntimeBrowserViewerInput(
	input RuntimeBrowserViewerInputPayload,
) bool {
	switch input.Kind {
	case "pointer":
	...
	case "keyboard":
		...
		case "press":
			return runtimeText(input.Key, 64) && input.Text == ""
		case "text":
			return input.Key == "" && runtimeText(input.Text, 4096) &&
...
func validateRuntimeBrowserViewerFrame(
	value RuntimeBrowserViewerFramePayload,
) error {
	if err := validateRuntimeAttemptIdentity(value.AttemptIdentity); err != nil ||
		!runtimeUUID(value.BrowserSessionID) ||
		!runtimeUUID(value.AttachmentID) ||
		...
		value.Width != 1280 ||
		value.Height != 720 {
		return errors.New("openlinker: invalid browser Viewer frame")
	}
	return nil
}

func validateRuntimeBrowserViewerFrameAck(
	value RuntimeBrowserViewerFrameAckPayload,
) error {
	if err := validateRuntimeAttemptIdentity(value.AttemptIdentity); err != nil ||
		value.ControlEpoch == 0 ||
		value.FrameSeq == 0 {
		return errors.New("openlinker: invalid browser Viewer frame acknowledgement")
	}
	return nil
}
